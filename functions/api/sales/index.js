...
export async function onRequestGet({ env }) {
  try {
    const { table, rows } = await readRows(env);
...
async function readRows(env) {
  ...
  for (const table of TABLE_CANDIDATES) {
    try {
      const order = await preferredOrder(env.DB, table);
  ...
  throw new Error(`No supported sales table found (${TABLE_CANDIDATES.join(', ')})`);
}
...
async function getWritableTable(env) {
  ...
  for (const table of TABLE_CANDIDATES) {
    try {
      ...
      if (columns.size) return { table, columns };
    } catch (error) {
      // Try the next likely table name.
    }
  }
...
async function preferredOrder(db, table) {
...
async function findById(db, table, id) {
  const result = await db.prepare(`SELECT * FROM ${quoteIdent(table)} WHERE "id" = ? LIMIT 1`).bind(id).first();
  return result || null;
}
